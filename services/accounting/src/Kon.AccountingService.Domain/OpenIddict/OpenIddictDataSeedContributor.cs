...
namespace Kon.AccountingService.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("AccountingService") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "AccountingService", DisplayName = "AccountingService API", Resources = { "AccountingService" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var blazorServerTieredClientId = configurationSection["AccountingService_BlazorServerTiered:ClientId"];
        if (!blazorServerTieredClientId.IsNullOrWhiteSpace())
        {
            var blazorServerTieredRootUrl = configurationSection["AccountingService_BlazorServerTiered:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["AccountingService_BlazorServerTiered:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["AccountingService_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["AccountingService_Swagger:RootUrl"]?.TrimEnd('/');
